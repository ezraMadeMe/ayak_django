...
from bokyak.models import MedicationCycle, MedicationRecord
from bokyak.serializer import MedicationCycleListSerializer, MedicationAlertSerializer, MedicationRecordSerializer
from .models import Hospital, Illness, User, MainIngredient, MedicationIngredient, Medication, UserMedicalInfo
...
class MainIngredientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'ingr_code', 'main_ingr_name_kr', 'main_ingr_name_en',
            'display_name', 'full_density_info', 'dosage_form',
            'main_ingr_density', 'main_ingr_unit', 'is_combination'
        ]


# MedicationIngredient 관련 시리얼라이저
class MedicationIngredientSerializer(serializers.ModelSerializer):
    ingredient_info = MainIngredientSerializer(source='ingredient', read_only=True)

    class Meta:
        model = MedicationIngredient
        fields = [
            'ingredient', 'ingredient_info', 'content_amount',
...
class MedicationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'class_name', 'dosage_form', 'is_prescription', 'ingredient_details'
        ]
...
class UserMedicalInfoSerializer(serializers.ModelSerializer):
    hospital_info = HospitalListSerializer(source='hospital', read_only=True)
    illness_info = IllnessListSerializer(source='illness', read_only=True)

    class Meta:
        model = UserMedicalInfo
        fields = [
            'id', 'hospital', 'illness', 'hospital_info',
            'illness_info', 'is_primary'
        ]
