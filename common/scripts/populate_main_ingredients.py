...
class MainIngredientImporter:
    ...
    def extract_korean_english_names(self, name_text, product_name=None, bar_code=None):
        ...
        # 영문명은 있지만 한글명이 없고 제품명이 있는 경우 API로 한글명 검색
        if product_name and self.api_key:
            logger.debug(f"API로 한글명 검색: {name_text} (제품명: {product_name})")
            ...
            if api_result and api_result[0]:  # 한글명이 있으면
                ...
                if api_result[1]:
                    api_english = api_result[1].strip()
                    # 괄호 제거
                    api_english = re.sub(r'\([^)]*\)', '', api_english).strip()
                    # (as xxx) 형태 제거
                    api_english = re.sub(r'\s*\(as\s+[^)]+\)', '', api_english).strip()
                    if api_english:
                        english_name = api_english

                logger.info(f"API에서 한글명 찾음: {english_name} → {korean_name}")
    ...
    def call_drug_api(self, bar_code):
        """의약품 API 호출"""
        if not self.api_key:
            return None
        ...
        try:
            ...
            logger.warning(f"API 호출 실패 ({bar_code}): {e}")
            return None
    ...
    def process_single_ingredient(self, code, standard_row, ingredient_df):
        ...
        # 성분 정보가 없고 API 키가 있는 경우 API로 검색
        if ingredient_entries.empty and self.api_key:
            logger.debug(f"API로 성분 정보 검색: {standard_row['표준코드']} (제품명: {standard_row['상품명']})")
            ...
            if api_result and api_result[0]:  # 한글명이 있으면
                ...
                if api_result[1]:
                    # 괄호 제거
                    name_clean = re.sub(r'\([^)]*\)', '', api_result[1]).strip()
                    # (as xxx) 형태 제거
                    english_name = re.sub(r'\s*\(as\s+[^)]+\)', '', name_clean).strip()

                logger.info(f"API에서 성분 정보 찾음: {english_name} → {korean_name}")
                time.sleep(0.5)  # API 호출 제한
                ...
                logger.warning(f"성분 정보를 찾을 수 없음: {standard_row['표준코드']}")
                return None
    ...
    def process_combination_drug(self, code, standard_row, ingredient_df):
        ...
        first_korean, first_english = self.extract_korean_english_names(
            first_ingredient_row['일반명'],
            standard_row['상품명'],
            standard_row['표준코드']
        )
        if first_english and not first_korean and pd.notna(standard_row['상품명']) and self.api_key:
            logger.debug(f"복합제 API 검색: {first_english} (제품명: {standard_row['상품명']})")
            api_result = self.call_drug_api(standard_row['표준코드'])
            if api_result and api_result[0]:
                api_korean_name = api_result[0]
                logger.info(f"복합제 API에서 한글명 찾음: {first_english} → {api_korean_name}")
                time.sleep(0.5)

        for idx, ingredient_row in ingredient_entries.iterrows():
            # 함량 정보
            ...
            # 첫 번째 성분이고 API에서 한글명을 찾았으면 사용
            if idx == 0 and api_korean_name:
...
# 사용 예시 (업데이트됨):
"""
# 전문의약품만, 주사제형 제외하여 임포트
python import_main_ingredients.py --standard-code-file "의약품표준코드2.xlsx" --ingredient-file "의약품주성분csv.xlsx" --api-key YOUR_API_KEY
python common/scripts/populate_main_ingredients.py --standard-code-file "common/data/의약품표준코드1.xlsx" --ingredient-file "common/data/의약품주성분csv.xlsx" --api-key %2Bbcu3KBpBKHn0BE4HiPANIoA27EbfZuLmewRjN3bBDwKW0W1CEDIOhCaN3FJyWghf%2BGqpEziBNcZF%2F3LAWO7mw%3D%3D

# 결과 검증
python manage.py shell
>>> from import_main_ingredients import validate_prescription_only, find_api_enhanced_ingredients, check_injection_exclusion, analyze_prescription_drugs
>>> validate_prescription_only()
>>> find_api_enhanced_ingredients()
>>> check_injection_exclusion()
>>> analyze_prescription_drugs()
"""
...
"""
# 기본 실행
python common/scripts/populate_main_ingredients.py --standard-code-file "common/data/의약품표준코드2.xlsx" --ingredient-file "common/data/의약품주성분csv.xlsx" --api-key +bcu3KBpBKHn0BE4HiPANIoA27EbfZuLmewRjN3bBDwKW0W1CEDIOhCaN3FJyWghf+GqpEziBNcZF/3LAWO7mw==
python common/scripts/populate_main_ingredients.py --standard-code-file "common/data/의약품표준코드2.xlsx" --ingredient-file "common/data/의약품주성분csv.xlsx" --api-key +bcu3KBpBKHn0BE4HiPANIoA27EbfZuLmewRjN3bBDwKW0W1CEDIOhCaN3FJyWghf+GqpEziBNcZF/3LAWO7mw==
